...
import { CalendarDays, Settings, TrendingUp, Car, Wrench, Users, DollarSign, BarChart3, Cog, ShoppingCart, MapPin, Upload, PieChart } from "lucide-react";
...
export default function App() {
  ...
  const [uploadedData, setUploadedData] = useState<{[location: string]: CSVRow[]}>({});
  const [isLoadingData, setIsLoadingData] = useState(false);
  const [allLocationsData, setAllLocationsData] = useState<{[location: string]: CSVRow[]}>({});
  ...
  useEffect(() => {
    const loadAllLocationsData = async () => {
      ...
      try {
        // Use the new getAllKPIData function for better performance
        const { data: allKpiData, error } = await getAllKPIData();
        
        if (!error && allKpiData.length > 0) {
  ...
  // Load specific location data when location changes
  useEffect(() => {
    const loadLocationData = async () => {
      if (allLocationsData[selectedLocation]) {
        // Data already loaded
        return;
      }
      ...
      setIsLoadingData(true);
      try {
        const { data: kpiData, error } = await getKPIDataByLocation(selectedLocation);
        
        if (error) {
          console.warn('Database connection issue:', error);
          // Don't show error to user, just use default data
        } else if (kpiData.length > 0) {
          // Convert KPI records back to CSV format for dashboard compatibility
          const csvData: CSVRow[] = kpiData.map((record: KPIRecord) => ({
            Tags: record.tags,
            Parameters: record.parameters,
            "Monthly Target": record.monthly_target,
            "Target MTD": record.target_mtd,
    ...
    loadLocationData();
  }, [selectedLocation, allLocationsData]);

  const handleDataUpload = (data: CSVRow[], location: string) => {
    setUploadedData(prev => ({
      ...prev,
      [location]: data
    }));
  ...
  const convertCSVToKPIData = (csvData: CSVRow[]) => {
    return csvData.map(row => ({
      ...
      target: row["Target MTD"] || row["Monthly Target"] || 0,
  ...
  // Get location-specific data from database only
  const getLocationData = (location: string) => {
    const locationCSVData = uploadedData[location];
      ...
      locations.forEach(location => {
        ...
        locationData.forEach(item => {
          ...
          if (existingItem) {
            ...
            existingItem.achievement = existingItem.target > 0 ? (existingItem.actual / existingItem.target) * 100 : 0;
          } else {
  ...
  const currentInflowData = allLocationData.filter(item => 
    item.name && (
  ...
  const currentLabourData = allLocationData.filter(item => 
    item.name && (
  ...
  const currentPartsData = allLocationData.filter(item => 
    item.name && (
  ...
  const currentEfficiencyData = allLocationData.filter(item => 
    item.name && (
  ...
  const totalThroughputMetrics = currentInflowData.find(item => 
  ...
  const totalLabourMetrics = currentLabourData.find(item => 
    item.name && item.name.toLowerCase().includes('total') && item.name.toLowerCase().includes('labour')
  ) || currentLabourData.reduce((acc, item) => ({
  ...
  const overallLabourAchievement = totalTargetLabour > 0 ? (totalActualLabour / totalTargetLabour) * 100 : 0;
  ...
  const efficiencyCounts = calculateCounts(currentEfficiencyData);
  
  // Calculate overall counts for overview
  const allCurrentData = [...currentInflowData, ...currentLabourData, ...currentPartsData, ...currentEfficiencyData];
  ...
  const getLocationMetrics = (location: string) => {
    const locationData = allLocationsData[location] || uploadedData[location];
    ...
    const throughputMetric = locationData.find(row => 
      row.Parameters && row.Parameters.toLowerCase().includes('total throughput')
    );
    
    const labourMetrics = locationData.filter(row => 
      row.Parameters && row.Parameters.toLowerCase().includes('labour') && 
      !row.Parameters.toLowerCase().includes('per ro')
    );
    
    const partsMetrics = locationData.filter(row => 
      row.Parameters && row.Parameters.toLowerCase().includes('parts') && 
      !row.Parameters.toLowerCase().includes('per ro')
    );

    // Calculate throughput metrics
    const throughputTarget = throughputMetric ? throughputMetric["Target MTD"] || throughputMetric["Monthly Target"] || 0 : 0;
    const throughputActual = throughputMetric ? throughputMetric["Actual As On Date"] || 0 : 0;
    const throughputAchievement = throughputMetric ? throughputMetric["% ACH"] || 0 : 0;

    // Calculate labour metrics (sum all labour-related entries)
    const labourTarget = labourMetrics.reduce((sum, row) => sum + (row["Target MTD"] || row["Monthly Target"] || 0), 0);
    const labourActual = labourMetrics.reduce((sum, row) => sum + (row["Actual As On Date"] || 0), 0);
    ...
    const partsTarget = partsMetrics.reduce((sum, row) => sum + (row["Target MTD"] || row["Monthly Target"] || 0), 0);
    const partsActual = partsMetrics.reduce((sum, row) => sum + (row["Actual As On Date"] || 0), 0);
  ...
  const aggregatedMetrics = {
    ...
    throughput: {
      target: locationMetrics.reduce((sum, metric) => sum + metric.throughput.target, 0),
      actual: locationMetrics.reduce((sum, metric) => sum + metric.throughput.actual, 0),
      achievement: 0
    },
    labour: {
      target: locationMetrics.reduce((sum, metric) => sum + metric.labour.target, 0),
      actual: locationMetrics.reduce((sum, metric) => sum + metric.labour.actual, 0),
      achievement: 0
    },
    parts: {
      target: locationMetrics.reduce((sum, metric) => sum + metric.parts.target, 0),
      actual: locationMetrics.reduce((sum, metric) => sum + metric.parts.actual, 0),
      achievement: 0
    },
    totalTarget: locationMetrics.reduce((sum, metric) => sum + metric.totalTarget, 0),
    totalAchieved: locationMetrics.reduce((sum, metric) => sum + metric.totalAchieved, 0)
  };
  ...
  const allLocationMetrics = [aggregatedMetrics, ...locationMetrics];
      ...
      <div className="max-w-[1400px] mx-auto space-y-4">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between mb-3">
                  <h1 className="text-2xl font-semibold text-gray-900">Arya Honda Dashboard</h1>
                ...
                <div className="flex items-center space-x-4 text-gray-600 text-sm">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{selectedLocation.toUpperCase()} Location</span>
                    {isLoadingData && (
                    ...
                    {!isLoadingData && selectedLocation !== "All Locations" && uploadedData[selectedLocation] && uploadedData[selectedLocation].length > 0 && (
                      <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-xs">
                        {uploadedData[selectedLocation].length} Records
                      </Badge>
                    )}
                  </div>
                </div>
        ...
        <Tabs defaultValue="directors" className="space-y-4">
          <TabsList className="grid w-full grid-cols-3 bg-white border border-gray-200 rounded-lg p-1">
            <TabsTrigger value="directors" className="flex items-center space-x-2 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
            ...
            <TabsTrigger value="upload" className="flex items-center space-x-2 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="bg-blue-50 p-4 rounded-lg text-center border border-blue-100">
                    <p className="text-2xl font-bold text-blue-600">
                      {aggregatedMetrics.throughput.achievement.toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-600">Total Throughput</p>
                    <p className="text-xs text-gray-500 mt-1">{aggregatedMetrics.throughput.actual.toLocaleString()} / {aggregatedMetrics.throughput.target.toLocaleString()}</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg text-center border border-green-100">
                    <p className="text-2xl font-bold text-green-600">
                      {aggregatedMetrics.labour.achievement.toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-600">Total Labour Sales</p>
                    <p className="text-xs text-gray-500 mt-1">₹{(aggregatedMetrics.labour.actual / 100000).toFixed(1)}L / ₹{(aggregatedMetrics.labour.target / 100000).toFixed(1)}L</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg text-center border border-purple-100">
                    <p className="text-2xl font-bold text-purple-600">
                      {aggregatedMetrics.parts.achievement.toFixed(1)}%
                    </p>
                    <p className="text-sm text-gray-600">Total Parts Sales</p>
                    <p className="text-xs text-gray-500 mt-1">₹{(aggregatedMetrics.parts.actual / 100000).toFixed(1)}L / ₹{(aggregatedMetrics.parts.target / 100000).toFixed(1)}L</p>
                  </div>
                  <div className="bg-orange-50 p-4 rounded-lg text-center border border-orange-100">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{Object.keys(allLocationsData).length} with data</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-gray-200 shadow-sm">
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center space-x-2 text-gray-900">
                  ...
                  <span>All Locations Performance Overview</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {/* Throughput Performance */}
                <div className="mb-8">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Vehicle Throughput Achievement</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {locationMetrics.filter(metric => metric.throughput.target > 0).map((metric) => (
                      <SpeedometerChart
                        key={`throughput-${metric.location}`}
                        location={metric.location}
                        achievement={metric.throughput.achievement}
                ...
                {/* Labour Performance */}
                <div className="mb-8">
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Labour Revenue Achievement</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {locationMetrics.filter(metric => metric.labour.target > 0).map((metric) => (
                      <SpeedometerChart
                        key={`labour-${metric.location}`}
                        location={metric.location}
                        achievement={metric.labour.achievement}
                ...
                {/* Parts Performance */}
                <div>
                  <h3 className="text-lg font-semibold text-gray-800 mb-4">Parts Sales Achievement</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    {locationMetrics.filter(metric => metric.parts.target > 0).map((metric) => (
                      <SpeedometerChart
          ...
          <TabsContent value="dashboard" className="space-y-4">
            ...
            <Tabs defaultValue="overview" className="space-y-4">
              <TabsList className="grid w-full grid-cols-5 bg-white border border-gray-200 rounded-lg p-1">
                <TabsTrigger value="overview" className="flex items-center space-x-1 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
                  <TrendingUp className="h-4 w-4" />
                  <span>Overview</span>
                </TabsTrigger>
                <TabsTrigger value="inflow" className="flex items-center space-x-1 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
                ...
                <TabsTrigger value="labour" className="flex items-center space-x-1 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
                ...
                <TabsTrigger value="parts" className="flex items-center space-x-1 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
                ...
                <TabsTrigger value="efficiency" className="flex items-center space-x-1 data-[state=active]:bg-blue-50 data-[state=active]:text-blue-700">
            ...
            {allLocationData.length > 0 ? (
                  ...
                  <CardTitle className="flex items-center justify-between text-gray-900">
                    ...
                    <div className="flex items-center space-x-2 text-sm">
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-green-700">{overallCounts.achieved} Achieved</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <div className="w-3 h-3 bg-yellow-500 rounded-full"></div>
                        <span className="text-yellow-700">{overallCounts.belowTarget} Below Target</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <div className="w-3 h-3 bg-red-500 rounded-full"></div>
                        <span className="text-red-700">{overallCounts.needsAction} Need Action</span>
                      </div>
                    </div>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {totalTargetThroughput > 0 && (
                      <div className="bg-orange-50 p-4 rounded-lg text-center border border-orange-100">
                        <p className="text-2xl font-bold text-orange-600">{overallThroughputAchievement.toFixed(1)}%</p>
                        <p className="text-sm text-gray-600">Throughput Achievement</p>
                        <p className="text-xs text-gray-500 mt-1">{selectedLocation}</p>
                      </div>
                    )}
                    {totalActualThroughput > 0 && (
                      <div className="bg-blue-50 p-4 rounded-lg text-center border border-blue-100">
                        <p className="text-2xl font-bold text-blue-600">{totalActualThroughput.toLocaleString()}</p>
                    ...
                    {totalActualLabour > 0 && (
                      <div className="bg-green-50 p-4 rounded-lg text-center border border-green-100">
                        <p className="text-2xl font-bold text-green-600">₹{(totalActualLabour / 100000).toFixed(1)}L</p>
                        <p className="text-sm text-gray-600">Labour Revenue</p>
                        <p className="text-xs text-gray-500 mt-1">{selectedLocation}</p>
                      </div>
                    )}
                    {totalTargetLabour > 0 && (
                      <div className="bg-purple-50 p-4 rounded-lg text-center border border-purple-100">
                        <p className="text-2xl font-bold text-purple-600">{overallLabourAchievement.toFixed(1)}%</p>
              ...
              <Card className="border-gray-200 shadow-sm">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-500">Please upload CSV data for {selectedLocation} to view executive summary.</p>
                </CardContent>
              </Card>
            )}
            ...
            {allLocationData.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {currentInflowData.find(item => item.name.toLowerCase().includes('total throughput')) && (
                  <KPICard
                ...
                {currentInflowData.find(item => item.name.toLowerCase().includes('pm throughput')) && (
                  <KPICard
                    title="PM Throughput"
                    actual={currentInflowData.find(item => item.name.toLowerCase().includes('pm throughput'))?.actual || 0}
                    target={currentInflowData.find(item => item.name.toLowerCase().includes('pm throughput'))?.target || 0}
                    unit="vehicles"
                    format="number"
                    trend="neutral"
                  />
                ...
                {currentEfficiencyData.find(item => item.name.toLowerCase().includes('conversion')) && (
                  <KPICard
                    title="Conversion Rate"
                    actual={currentEfficiencyData.find(item => item.name.toLowerCase().includes('conversion'))?.actual || 0}
                    target={currentEfficiencyData.find(item => item.name.toLowerCase().includes('conversion'))?.target || 0}
                    unit=""
                    format="percentage"
                    trend="neutral"
                  />
            ...
            {/* Performance Charts */}
            {allLocationMetrics.some(metric => metric.totalTarget > 0) && (
              <div className="grid grid-cols-1 gap-4">
                <PerformanceChart
                  title="Target Achieved - All Locations (₹ in Lakhs)"
                  data={allLocationMetrics.filter(metric => metric.location !== "All Locations").map(metric => ({
                ...
                {currentInflowData.length > 0 ? (
                  ...
                  <Card className="border-gray-200 shadow-sm">
                    <CardContent className="p-8 text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">No Inflow Data Available</h3>
                      <p className="text-gray-500">Please upload CSV data for {selectedLocation} to view inflow metrics.</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
                ...
                {currentLabourData.length > 0 ? (
                  ...
                  <Card className="border-gray-200 shadow-sm">
                    <CardContent className="p-8 text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">No Labour Data Available</h3>
                      <p className="text-gray-500">Please upload CSV data for {selectedLocation} to view labour metrics.</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
                ...
                {currentPartsData.length > 0 ? (
                  ...
                  <Card className="border-gray-200 shadow-sm">
                    <CardContent className="p-8 text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">No Parts Data Available</h3>
                      <p className="text-gray-500">Please upload CSV data for {selectedLocation} to view parts metrics.</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
                ...
                {currentEfficiencyData.length > 0 ? (
                  ...
                  <Card className="border-gray-200 shadow-sm">
                    <CardContent className="p-8 text-center">
                      ...
                      <h3 className="text-lg font-medium text-gray-600 mb-2">No Efficiency Data Available</h3>
                      <p className="text-gray-500">Please upload CSV data for {selectedLocation} to view efficiency metrics.</p>
                    </CardContent>
                  </Card>
                )}
              </TabsContent>
          ...
          <TabsContent value="upload" className="space-y-4">
            <CSVUpload onDataUpload={handleDataUpload} uploadedData={uploadedData} />
          </TabsContent>
        </Tabs>
      </div>
    </div>
