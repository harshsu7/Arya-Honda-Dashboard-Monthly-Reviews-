...
export default function App() {
  ...
  const convertCSVToKPIData = (csvData: CSVRow[]) => {
    return csvData.map(row => ({
      ...
      target: row["Monthly Target"] || row["Target MTD"] || 0,
  ...
  const getLocationMetrics = (location: string) => {
    ...
    const throughputTarget = throughputMetric ? throughputMetric["Monthly Target"] || throughputMetric["Target MTD"] || 0 : 0;
    ...
    const labourTarget = labourMetrics.reduce((sum, row) => sum + (row["Monthly Target"] || row["Target MTD"] || 0), 0);
    ...
    const partsTarget = partsMetrics.reduce((sum, row) => sum + (row["Monthly Target"] || row["Target MTD"] || 0), 0);
