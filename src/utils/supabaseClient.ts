...
export interface KPIRecord {
  ...
  location: string;
  month: string; // Format: "July", "August", etc. or "2024-07"
  uploaded_at?: string;
  file_name?: string;
  created_at?: string;
  updated_at?: string;
...
// Function to delete KPI data by month (for month-wise uploads)
export const deleteKPIDataByMonth = async (month: string): Promise<{ success: boolean; error?: string }> => {
  try {
    const { error } = await supabase
      .from('kpi_data')
      .delete()
...
// Function to get KPI data by month
export const getKPIDataByMonth = async (month: string): Promise<{ data: KPIRecord[]; error?: string }> => {
  try {
    const { data, error } = await supabase
      .from('kpi_data')
      .select('*')
    ...
    console.error('Error fetching KPI data by month:', error);
    return { data: [], error: error instanceof Error ? error.message : 'Unknown error' };
  }
};

// Function to get KPI data by location and month
export const getKPIDataByLocationAndMonth = async (location: string, month: string): Promise<{ data: KPIRecord[]; error?: string }> => {
  try {
    const { data, error } = await supabase
      .from('kpi_data')
      .select('*')
...
// Function to get available months from database
export const getAvailableMonths = async (): Promise<{ data: string[]; error?: string }> => {
  try {
    const { data, error } = await supabase
      .from('kpi_data')
      .select('month')
...
// Function to get KPI data by multiple locations and months
export const getKPIDataByFilters = async (locations: string[], months: string[]): Promise<{ data: KPIRecord[]; error?: string }> => {
  try {
    let query = supabase.from('kpi_data').select('*');
    ...
    console.error('Error fetching filtered KPI data:', error);
    return { data: [], error: error instanceof Error ? error.message : 'Unknown error' };
  }
};