...
import { saveKPIData, saveUploadSession, getUploadSessions, deleteKPIDataByLocation, type KPIRecord, type UploadSession } from "../utils/supabaseClient";
...
export function CSVUpload({ onDataUpload, uploadedData }: CSVUploadProps) {
  ...
  const parseCSVWithPapa = (file: File, locationFromSelect: string): Promise<{ data: CSVRow[]; errors: string[] }> => {
        ...
        complete: (results) => {
          ...
          results.data.forEach((row: any, index: number) => {
            ...
            if (validation.isValid) {
              // Ensure Location field exists and default to selected location if not present
              row.Location = row.Location && locations.includes(row.Location) ? row.Location : locationFromSelect;
  ...
  const handleFileUpload = useCallback(async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (file.type === 'text/csv' || file.name.endsWith('.csv')) {
        ...
        try {
          const { data, errors } = await parseCSVWithPapa(file, selectedLocation);
          ...
          if (data.length > 0) {
            // Determine the final location from the parsed data
            const locationInCSV = data[0]?.Location;
            finalLocation = locationInCSV || selectedLocation;
            
            // Delete existing data for this specific location before inserting new data
            const deleteResult = await deleteKPIDataByLocation(finalLocation);
            if (!deleteResult.success) {
              uploadErrors.push(`Warning: Could not clear existing data for ${finalLocation} - ${deleteResult.error}`);
            }
            ...
            const kpiRecords: KPIRecord[] = data.map(row => ({
              ...
              location: row.Location || finalLocation,
              file_name: file.name,
            ...
            if (saveResult.success) {
              ...
              onDataUpload(data, finalLocation);
            } else {
