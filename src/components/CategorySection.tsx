...
export function CategorySection({ title, items, color = "blue", location }: CategorySectionProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Performance Chart - only show if we have multiple items with valid data */}
      {items.length > 2 && items.some(item => item.actual > 0) && (
        <CategoryChart 
          title={title}
          items={items}
          format={title.toLowerCase().includes('labour') || title.toLowerCase().includes('parts') ? 'currency' : 'number'}
        />
      )}
            ...
            {items.map((item, index) => (
              <div key={index} className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
                ...
                <div className="grid grid-cols-4 gap-4 text-sm">
                  ...
                  {item.targetMTD && (
                    <div>
                      <p className="text-muted-foreground">Target MTD</p>
                      <p className="font-semibold text-blue-600">{formatNumber(item.targetMTD, item.unit)}</p>
                    </div>
                  )}
                  <div>
