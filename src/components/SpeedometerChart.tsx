...
export function SpeedometerChart({ location, achievement, target, actual, title }: SpeedometerChartProps) {
  ...
  const getColor = (achievement: number) => {
    if (achievement >= 100) return "#26b159ff"; // green
    if (achievement >= 90) return "#051df5ff"; // blue  
    if (achievement >= 70) return "#ffd20cff"; // yellow
    return "#d31313ff"; // red
  };
  ...
  return (
    <Card className="border-gray-200 shadow-sm h-full">
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-black-500 text-center">{title}</p>
      </CardHeader>
      <CardContent className="pt-0">
        <div className="relative w-32 h-20 mx-auto mb-3">
          ...
          <svg className="w-full h-full" viewBox="0 0 128 80" style={{ overflow: 'visible' }}>
            ...
            />
            {/* Center point */}
            <circle cx="64" cy="56" r="3" fill="#374151" />
            {/* Needle */}
            <line
              x1="64"
              y1="56"
